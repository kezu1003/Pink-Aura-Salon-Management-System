...
export async function appointmentsOverview(req, res) {
  try {
    ...
    // Unify field names from different schemas (startTime vs start, staff vs staffId, service vs serviceId)
    const pipeline = [
      {
        $project: {
    ...
    const totalsAgg = await mongoose.connection
    ...
    const byStaffStats = await mongoose.connection
      .collection("appointments")
      .aggregate([
        ...pipeline,
    ...
    //  Per-staff service breakdown
  
    const byStaffServicesAgg = await mongoose.connection
      .collection("appointments")
      .aggregate([
        ...pipeline,
    ...
    // Top services 
    const topServices = await mongoose.connection
      .collection("appointments")
      .aggregate([
        ...pipeline,
    ...
    // Peak Hours and Days 
    const peaksAgg = await mongoose.connection
      .collection("appointments")
      .aggregate([
        ...pipeline,
        { $match: { status: { $ne: "cancelled" } } },
        { $project: { hour: { $hour: "$ts" }, day: { $dayOfWeek: "$ts" } } }, // 1=Sun..7=Sat
        {
          $facet: {
