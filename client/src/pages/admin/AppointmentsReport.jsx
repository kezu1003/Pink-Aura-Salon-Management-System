...
export default function AppointmentsReport() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [st, sv] = await Promise.all([api.get("/api/staff"), api.get("/api/services")]);
  ...
  async function load() {
    try {
      const qs = new URLSearchParams(Object.fromEntries(Object.entries(filters).filter(([_, v]) => v)));
      const { data } = await api.get("/api/reports/appointments?" + qs.toString());
      setReport(data);
    } catch (e) {
  ...
  function downloadCSV() {
    ...
    (report.topServices || []).forEach(s => lines.push(`${s.serviceName},${s.count}`));

    lines.push("");

    lines.push("By Staff");
    lines.push("Staff,Total,Booked,Confirmed,Rescheduled,Completed,Cancelled");

    (report.byStaff || []).forEach(s => {

      const st = s.statuses || {};
      lines.push(`${s.staffName || s.staffId},${s.total || 0},${st.booked || 0},${st.confirmed || 0},${st.rescheduled || 0},${st.completed || 0},${st.cancelled || 0}`);
      (s.services || []).forEach(x => lines.push(`  - ${x.serviceName},${x.count}`));
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="rounded-2xl border border-pink-200 bg-white p-4 shadow-sm">
        <div className="mb-3 text-lg font-semibold text-black">Appointment Overview</div>
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <label className="block text-xs text-black/60">From</label>
            <input type="date" className="rounded-xl border border-black/10 p-2" value={filters.from}
                   onChange={e => setFilters({ ...filters, from: e.target.value })} />
          </div>
          <div>
            <label className="block text-xs text-black/60">To</label>
            <input type="date" className="rounded-xl border border-black/10 p-2" value={filters.to}
                   onChange={e => setFilters({ ...filters, to: e.target.value })} />
          </div>
          <div>
            <label className="block text-xs text-black/60">Service</label>
            <select className="rounded-xl border border-black/10 p-2" value={filters.serviceId}
          ...
          <div>
            ...
            <select className="rounded-xl border border-black/10 p-2" value={filters.staffId}
                ...
                {(report.byStaff || []).map(s => {
                  ...
                  return (
                    <tr key={s.staffId} className="border-t">
                      ...
                      <td className="p-2">{top.map(x => `${x.serviceName} (${x.count})`).join(", ")}</td>
                    </tr>
                  );
                })}
              </tbody>
