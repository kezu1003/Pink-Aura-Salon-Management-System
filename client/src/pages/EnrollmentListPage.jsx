...
const EnrollmentListPage = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      // Note: Pie chart moved to separate reports page
          ...
          <div className="mt-12 bg-gradient-to-r from-[#FEF4F1] to-white rounded-3xl shadow-lg border-2 border-[#FBAA99]/20 p-8" data-scroll-animation>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                ...
                { icon: <BarChart3 className="w-6 h-6" />, label: "View Analytics", desc: "Statistics", color: "from-[#4D423A] to-[#000000]", link: "/enrollments/reports" },
              ].map((action, index) => (
                <Link
                  key={index}
                  to={action.link}
