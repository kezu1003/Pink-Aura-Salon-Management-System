...
const CourseDetailsPage = () => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      doc.setFontSize(22);
      doc.setFont(undefined, 'bold');
      doc.text('Course Details Report', pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 15;
      ...
      doc.setFontSize(18);
      doc.setFont(undefined, 'bold');
      doc.text(course.courseName || 'Course Name', pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 25;
      ...
      // Course details with enhanced formatting
      const details = [
      ...
      const fileName = `Pink-Aura-Course-Report-${course.courseName?.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'course'}-${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(fileName);
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute('download', `course-report-${course.courseName?.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'course'}-${Date.now()}.csv`);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#FEF4F1] to-white flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-[#4D423A] font-medium">Loading course details...</p>
        </div>
      </div>
    );
  }
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#4D423A]/70 mt-1">View and edit course information</p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-r from-[#FBAA99] to-[#4D423A] p-8 text-white relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/90">Manage course information and settings</p>
                </div>
            ...
            <div className="p-8">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {formFields.map((field, index) => (
                  <div key={field.field} className={`${field.isTextarea ? 'lg:col-span-2' : ''}`}>
                      ...
                      {isEditing ? (
                        field.isTextarea ? (
                          <textarea
                            ...
                            value={course?.[field.field] || ''}
                            onChange={(e) => setCourse({ ...course, [field.field]: e.target.value })}
                        ...
                        <div className={`w-full px-6 py-4 border-2 border-[#FEF4F1] rounded-2xl text-lg bg-[#FEF4F1]/30 text-[#4D423A] ${field.isTextarea ? 'min-h-32' : 'h-14'} flex items-start`}>
                          <span className={field.isTextarea ? 'pt-0' : 'pt-1'}>
                            {course?.[field.field] || `No ${field.label.toLowerCase()} provided`}
                          </span>
                        </div>
                      )}
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-[#FEF4F1] p-6">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-xl font-bold text-[#4D423A]">Course Stats</h3>
              </div>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-[#4D423A]/70">Enrolled Students</span>
                  <span className="font-semibold text-[#4D423A]">24</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#4D423A]/70">Completion Rate</span>
                  <span className="font-semibold text-[#FBAA99]">92%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#4D423A]/70">Average Rating</span>
                  <span className="font-semibold text-[#4D423A]">4.8/5</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-[#FEF4F1] p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[#4D423A]">Active Course</span>
                </div>
